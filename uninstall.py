...
def generate_report():
    ...
    reports_dir = os.path.join(frappe.get_site_path(), 'private', 'files', 'dfp_external_storage_reports')
    ...
    with open(report_path, 'w') as report_file:
        # Write header
        report_file.write("File ID,File Name,Storage Type,Storage Name,External Path,File Size,Is Private,Content Hash\n")
        ...
        # Write file details
        for f in files:
            report_file.write(f'"{f.name}","{f.file_name}","{f.storage_type}","{f.dfp_external_storage}",')
            report_file.write(f'"{f.dfp_external_storage_s3_key}",{f.file_size},{1 if f.is_private else 0},"{f.content_hash}"\n')
