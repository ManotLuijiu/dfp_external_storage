...
class DFPS3BucketList {
  ...
  diagnoseConnection() {
    ...
    frappe.call({
      method:
        "dfp_external_storage.dfp_external_storage.page.dfp_s3_bucket_list.dfp_s3_bucket_list.diagnose_storage_connection",
      args: {
  ...
  refresh_files() {
    // Make sure we get a valid template value
    let template;
    try {
      template = this.template.get_value();
      if (!template) {
        template = "list"; // Set a default value
        // frappe.throw(__('Template is required'));
      }
    } catch (e) {
      template = "list"; // Fallback
      console.error("Error getting template value:", e);
    ...
    if (!storage) {
      try {
        storage = this.storage.get_value();
      } catch (e) {
    ...
    let file_type;
    try {
      file_type = this.file_type.get_value() || "all";
    } catch (e) {
      file_type = "all"; // Fallback
    ...
    frappe.call({
        ...
        try {
          ...
          this.$content.html(
            frappe.render_template(template, {
              ...
              fileSizeToHumansMode: this.fileSizeToHumansMode,
            })
          );
        } catch (e) {
      ...
      error: (err) => {
        ...
        frappe.msgprint({
			title: __('Error'),
			indicator: 'red',
			message: __('Error fetching files: ') + (err.message || 'Unknown error')
		});
      },
    });
  }
